...
BLECharacteristic *HealthTickCharacteristic = NULL;

class HealthTickCharacteristicCallbacks : public BLECharacteristicCallbacks
{
  void onWrite(BLECharacteristic *HealthTickCharacteristic)
  {
    Serial.println("[HEALTH_CHECK]  Running onWrite callback");

    std::string recievedValue = HealthTickCharacteristic->getValue();
...
void health_setup(BLEService *pService)
{
  ...
  HealthTickCharacteristic = pService->createCharacteristic(
      CHARACTERISTIC_UUID_HEALTH, BLECharacteristic::PROPERTY_READ |
                                      BLECharacteristic::PROPERTY_WRITE |
                                      BLECharacteristic::PROPERTY_NOTIFY |
                                      BLECharacteristic::PROPERTY_INDICATE);

  HealthTickCharacteristic->setCallbacks(new HealthTickCharacteristicCallbacks());

  // https://www.bluetooth.com/specifications/gatt/viewer?attributeXmlFile=org.bluetooth.descriptor.gatt.client_characteristic_configuration.xml
  // Create a BLE Descriptor
  HealthTickCharacteristic->addDescriptor(new BLE2902());
...
void health_tick()
{
  ...
  HealthTickCharacteristic->setValue((uint8_t *)&health_tick_value, 4);
  HealthTickCharacteristic->notify(); // Notify connected devices (if desired)
